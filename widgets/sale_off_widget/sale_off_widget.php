...
class phoen_sale_off_wpb_widget extends WP_Widget 
{
	function __construct() 
	{
		parent::__construct(
		// Base ID of your widget
		'phoen_sale_off_wpb_widget', 

		// Widget name will appear in UI
		__('Woocommerce Sale Off by Phoeniixx', 'wpb_widget_domain'), 

		// Widget description
		array( 'description' => __( 'Sale Off Products For Woocommerce', 'wpb_widget_domain' ), 'panels_groups' => array('phoeniixx')) 
		);
	}
	...
	public function widget( $args, $instance )
	{
		...
		<div class="recent-products woocommerce">
			<h2 class="recent-products-heading"><?php echo $title; ?></h2>  
				<ul id="<?php echo 'phoen_sale_product'.$instance['panels_info']['id']; ?>" class="products owl-carousel" style="display:block;">
				<?php
			...
			<script>
				...
				jQuery(document).ready(function(){
					...
					jQuery('.panel-grid').each(function(){
						...
						jQuery(this).find('#phoen_sale_product'+<?php echo $instance['panels_info']['id']; ?>).owlCarousel({
						...
						jQuery(this).find('#phoen_sale_product'+<?php echo $instance['panels_info']['id']; ?>).show();
...
//echo $args['after_widget'];
	...
	public function form( $instance ) 
	{
		...
		{
			$title = __( 'Sale Off', 'wpb_widget_domain' );
		}
		if ( isset( $instance[ 'product_limit' ] ) ) 
		{
			$product_limit = $instance[ 'product_limit' ];
	...
	public function phoen_sale_off_pro_update( $new_instance, $old_instance )
	{
		$instance = array();
...
function phoen_sale_off_pro_wpb_load_widget() {
	register_widget( 'phoen_sale_off_wpb_widget' );
}
add_action( 'widgets_init', 'phoen_sale_off_pro_wpb_load_widget' );
?>